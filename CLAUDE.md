...
27. **`_safe_date()` handles relative date strings and never returns `None`** — Indeed returns `"15 days ago"`, Monster returns `"Today"` / `"1 day ago"`, Glassdoor returns `"4d"` / `"24h"`. `_safe_date()` in `backend/scrapers/base.py` resolves all these patterns to real `date` objects using regex + `timedelta`. Final fallback is `today` (not `None`) so `posted_date` is always populated. Do not remove this logic.
...
29. **Frontend falls back to `scraped_at` when `posted_date` is null** — `signals.js` shows `~scraped_at` (tilde = approximate); `company/[name].js` shows `scraped_at` without prefix. Both fields are in the API response. Posted date is mandatory UX — never show `—` if `scraped_at` exists.
